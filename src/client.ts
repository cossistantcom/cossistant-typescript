...
export interface ClientOptions {
  /**
   * Defaults to process.env['COSSISTANT_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['COSSISTANT_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['COSSISTANT_LOG'] or 'warn' if it isn't set.
   */
...
export class Cossistant {
   ...
   * API Client for interfacing with the Cossistant API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['COSSISTANT_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['COSSISTANT_BASE_URL'] ?? https://api.cossistant.com/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('COSSISTANT_BASE_URL'),
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.cossistant.com/v1`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Cossistant.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('COSSISTANT_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.cossistant.com/v1';
  }
...
Cossistant.Organizations = Organizations;
Cossistant.Websites = Websites;
Cossistant.Messages = Messages;
Cossistant.Conversations = Conversations;
Cossistant.Visitors = Visitors;
Cossistant.Contacts = Contacts;
Cossistant.Uploads = UploadsAPIUploads;
Cossistant.Knowledge = Knowledge;
